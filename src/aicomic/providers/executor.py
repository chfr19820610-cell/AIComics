...
def execute_provider_requests(
    ...
    for request_item in provider_requests.get("requests", []):
        ...
        preview = build_provider_request_preview(request_item, providers_config_path)
        provider_ready = resolve_provider_ready(provider, preview, api_key_ready)
        if provider_ready:
        ...
        try:
            execution_output = perform_provider_request(request_item, providers_config_path)
